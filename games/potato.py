...
class Game:
    ...
    async def game_process(self):
        ...
        await self.bot.send_log(log_type="GameEnd", info=f"Игра закончилась", member=self.bot.get_user(list(self.players_list.keys())[0]), fields=("Победитель:", f"<@{first}>"), color=0xE160F9)
